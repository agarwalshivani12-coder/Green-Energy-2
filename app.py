...
for t in range(steps): 
              total_energy = solar[t] + wind[t] + battery * 0.1
...
with col1: 
         ...
         fig1 = go.Figure() 
             ...
             fig1.add_trace(go.Scatter(y=workload_series, name="Workload")) 
             st.plotly_chart(fig1, use_container_width=True)

with col2: 
             st.subheader("Battery Level") 
             fig2 = go.Figure() 
                 fig2.add_trace(go.Scatter(y=battery_series, name="Battery")) 
...
col3.metric("Final Workload", round(workload_series[-1], 2)) 
                     col4.metric("Final Battery", round(battery_series[-1], 2))
                     col5.metric("Grid Stability", f"{round(grid_stability[-1]*100,2)}%")
